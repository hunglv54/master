...
\begin{document}
	...
	\chapter{Phương pháp chuyển đổi}				
		...
		\begin{megaalgorithm}[H]
			...
			\DontPrintSemicolon
			\Input{Một tập hợp A}
			\Output{Tập hợp các phần tử của tập hợp A đã được mã hóa.}
			...
			\For{mỗi phần tử $a_{i}$ trong tập A}{
				...
				Lưu $x$ vào bảng ánh xạ ứng với vị trí của phần tử $a_i$.
			}
		...
		\begin{megaalgorithm}[H]
			...
			\Output{Tập hợp các chuyển trạng thái đã được mã hóa}
				
			\For{mỗi chuyển trạng thái được biểu diễn $\delta(q, w, q')$}{
				Lấy dạng biểu diễn $x_i$ của $q$ từ bảng ánh xạ của tập các trạng thái đầu vào\;
				Lấy dạng biểu diễn $e_i$ của $w$ từ bảng ánh xạ của tập các sự kiện\;
				Lấy dạng biểu diễn $x_{i+1}$ của $q'$ từ bảng ánh xạ của tập các trạng thái đầu ra\;
