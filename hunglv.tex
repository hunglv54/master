...
\begin{document}
	...
	\chapter{Kiến thức cơ sở}
		...
		N = $<\textrm{X}, \textrm{E}, \tau(\textrm{X}, \textrm{E}, \textrm{X}'), \iota(\textrm{X})>$\\
		Trong đó:\\
		\begin{itemize}
		...
		Với dạng đặc tả sử dụng hàm logic N = $<\textrm{X}, \textrm{E}, \tau(X, E, X'), \iota(\textrm{X})>$. Trong đó:\\
		\begin{itemize}
		...
		Cho dạng đặc tả sử dụng hàm logic N = <X, E, $\tau(\textrm{X}, \textrm{E}, \textrm{X}')$, $\iota$(X)>. Trong đó:\\
		\begin{itemize}
			\item X = $\{x_1, x_2\}$, X' = $\{x_5, x_6\}$,
			\item E = $\{x_3, x_4\}$,
	...
	\chapter{Phương pháp chuyển đổi}				
		...
		Trong đó, $\gamma^i$ là phép gán cho hàm $e^i$ biểu diễn hành vi thứ i của hệ thống. Dựa vào bảng ánh xạ Map ta chuyển được $\xi$ thành $\sigma = w_0w_1...w_n$. Ta sẽ chứng minh $\sigma$ được đoán nhận bởi L(M) hay $\sigma$ là một vết của M. Thật vậy, theo định nghĩa vết của dạng đặc tả đặc tả sử dụng hàm logic 2.10 vì $\xi = \gamma^0\gamma^1...\gamma^n$ là một vết của N nên tồn tại các phép gán $\upsilon^0$, $\upsilon^1$, ..., $\upsilon^n$, $\gamma^0$, $\upsilon^1$, ..., $\upsilon^n$ sao cho $\iota[\upsilon^0]$ = T và $\tau[\upsilon^i, \gamma^i, \upsilon^{i+1}]$ = T với $\forall i$: $0 \leq i < n$ mà $\upsilon^0$ là phép gán cho $\alpha_0$ trên tập X, dựa vào bảng ánh xạ $\alpha_0$ tương ứng với $q_0$ nên $q_0$ là trạng thái khởi tạo của M. Thêm vào đó, vì $\tau[\upsilon^i, \gamma^i, \upsilon^{i+1}]$ = T nên $\upsilon^i$ = T, $\gamma^i$ = T, $\upsilon^{i+1}$ = T. Lại có, $\upsilon^i$ là phép gán cho $\alpha_i$ trên tập X, $\upsilon^{i+1}$ là phép gán cho $\alpha_{i+1}$ trên tập X', $\gamma^i$ là phép gán cho $e_i$ trên tập E nên ta có chuyển trạng thái $\tau(X, E, X') = \tau(\alpha^i, \gamma^i, \alpha^{i+1})$, dựa vào thành phần $\delta \mapsto \tau$ của bảng ánh xạ Map, $\tau(\alpha_i, e_i, \alpha_{i+1})$ sẽ tương ứng với $\delta(q_i, w_i, q_{i+1})$. Với $q_0$ là trạng thái khởi tạo, xét một chuỗi $\sigma = w_0w_1...w_i$ với $\forall i$: $0 \leq i < n$ ta có với sự kiện $w_0$, từ sự kiện ban đầu của hệ thống là $q_0$ hệ thống chuyển sang trạng thái $q_1$ (do sự tồn tại của chuyển trạng thái $\delta(q_0, w_0, q_1)$), từ trạng thái $q_1$ khi xảy ra sự kiện $w_1$ hệ thống chuyển sang trạng thái $q_2$ (do sự tồn tại của chuyển trạng thái $\sigma(q_1, w_1, q_2)$), một cách tương tự với các sự kiện từ $w_2$ đến $w_{n-1}$, sau khi sự kiện $w_{n-1}$ xảy ra hệ thống chuyển sang trạng thái $q_i$, khi sự kiện $w_i$ xảy ra, do sự tồn tại của chuyển trạng thái $\sigma(q_i, w_i, q_{i+1})$ nên hệ thống chuyển sang trạng thái $q_{i+1}$. Vì thế, với $\forall i$: $0 \leq i < n$, tồn tại trạng thái $q_i \in$ Q sao cho $q_0 \overset{\sigma}{\longrightarrow} q_{i}$ nên theo định nghĩa 2.4, $\sigma = w_0w_1...w_n$ là một vết của M.
